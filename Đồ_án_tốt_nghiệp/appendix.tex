...
không có khối \texttt{self\_play}: khi khai báo khối này, ML-Agents phát sinh
...
\smallskip
$^{\ddagger}$ Riêng DQN trên Football Table phải tắt self-play, do ML-Agents phát sinh lỗi khi kết hợp trainer off-policy với ghost trainer. Vì vậy lần chạy này không có chỉ số ELO và không so sánh trực tiếp được với bốn thuật toán còn lại trên cùng môi trường.

\smallskip
Ghi chú chung: mọi lần chạy đều dùng $8$ training area song song trong mỗi scene. Reward là
phần thưởng trung bình mười phần trăm cuối, cùng định nghĩa dùng trong
Chương~\ref{ch:results}. Mỗi tổ hợp chỉ có một seed, nên bảng ghi số liệu mô tả của từng
lần chạy, không phải trung bình hay thứ hạng thống kê giữa nhiều lần lặp.}
\end{table}
...
tại kho mã nguồn của đề tài: \url{https://github.com/VanHuyen193/GraduationProject.git}
