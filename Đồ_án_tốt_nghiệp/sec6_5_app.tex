...
\indent Các chương trước đã trình bày quá trình huấn luyện và so sánh bốn thuật toán
...
PPO, SAC, MA-POCA, MAPPO hoặc các phiên bản huấn luyện khác nhau) cho tác nhân sử
...
\indent Nhóm thứ tư là yêu cầu tra cứu kết quả huấn luyện ngay trong ứng dụng. Thay vì
...
khi ứng dụng khởi động thông qua thuộc tính
\texttt{[RuntimeInitializeOnLoadMethod]}; khi scene môi trường được nạp xong, thành
phần này kiểm tra xem người dùng có đi vào từ menu hay không. Nếu có, nó tìm các tác
...
\texttt{BehaviorParameters} quy định nguồn ra quyết định: ở chế độ người chơi (và ở
...
\caption{Tab \textit{CHƠI} của menu chính: khung xem trước ba chiều (trái) và các lựa
...
\texttt{Resources/\allowbreak EnvPrefabs} và sao chép \emph{chỉ} các cặp lưới - vật liệu
...
\caption{Khu vực chọn mô hình khi Football Table ở chế độ \textit{Agent tự chơi}:
...
ứng dụng được đóng gói sẵn chín mô hình, liệt kê trong Bảng~\ref{tab:app_models}, bao
phủ kết quả huấn luyện tốt nhất của từng thuật toán trên từng môi trường.
...
\begin{tabular}{|p{3.4cm}|p{5.4cm}|p{3.0cm}|>{\centering\arraybackslash}p{1.8cm}|}
\hline
...
\multirow{2}{*}{Capture The Flag}
 & \texttt{ctf\_poca\_v2} & MA-POCA & 1,68M \\
\cline{2-4}
 & \texttt{ctf\_mappo\_v2} & MAPPO & 1,68M \\
\hline
\multirow{3}{*}{Football Table}
 & \texttt{FB01} & PPO (self-play) & 1,59M \\
\cline{2-4}
 & \texttt{Football\_SAC\_01} & SAC & 1,59M \\
\cline{2-4}
 & \texttt{Football\_POCA\_01} & MA-POCA & 1,59M \\
\hline
\end{tabular}
\end{table}
...
của TensorBoard và thực hiện bốn bước xử lý. Thứ nhất, kịch bản gộp chuỗi
\texttt{Environment/Cumulative Reward} của tất cả tệp nhật ký thuộc cùng một lần chạy
và sắp xếp theo số bước. Thứ hai, chuỗi được làm mượt bằng trung bình trượt để loại
bớt nhiễu giữa các tập. Thứ ba, chuỗi đã làm mượt được rút gọn xuống tối đa một trăm
hai mươi điểm cách đều nhằm giữ kích thước tệp nhỏ, phù hợp để nạp trong game. Thứ tư,
kịch bản tính các thống kê tóm tắt gồm phần thưởng cuối, phần thưởng tốt nhất, trung
...
(giá trị phần thưởng đã làm mượt). Bên phía Unity, lớp \texttt{TrainingDataStore} nạp
...
dùng thống nhất cho cả biểu đồ, chú giải và bảng. Vì mã lần chạy được lưu kèm, mọi con
số hiển thị trong ứng dụng đều truy vết được về đúng thư mục kết quả đã dùng để phân
tích ở Chương~\ref{ch:results}.\par}
...
\caption{Tab \textit{THÔNG SỐ}: đường cong phần thưởng tích lũy và bảng thống kê của
...
đến đâu. Cần lưu ý rằng đại lượng được vẽ ở đây là phần thưởng tích lũy môi trường
(\texttt{Environment/Cumulative Reward}) lấy nguyên từ nhật ký huấn luyện, nên thang
giá trị có thể khác với các đường cong phần thưởng ngoại sinh đã trình bày ở
Chương~\ref{ch:results}; hai tab số liệu trong ứng dụng đóng vai trò tra cứu nhanh chứ
không thay thế phần phân tích chi tiết của báo cáo.
...
chiều cao cột được chuẩn hóa theo giá trị lớn nhất \emph{trong từng thẻ} chứ không
...
Cần lưu ý rằng bảng xếp hạng trong ứng dụng sắp thứ tự thuần theo phần thưởng, nên với
môi trường Football Table nó không trùng với thứ hạng ở Bảng~\ref{tab:overall_ranking}
- nơi thứ hạng của môi trường đối kháng được xếp theo mức tăng chỉ số ELO như đã lập
luận ở Mục~\ref{ch:results}.

\begin{figure}[H]
\centering
\includegraphics[width=0.97\textwidth]{image/app/app_menu_compare.png}
\caption{Tab \textit{SO SÁNH}: biểu đồ cột theo từng môi trường và bảng xếp hạng
thuật toán}
...
\hline
\multicolumn{5}{|p{14.4cm}|}{$^{*}$ Hai chế độ này cần scene Cross The Road chứa từ
...
\caption{Chế độ \textit{Agent tự chơi} trong Cross The Road: mô hình
...
\caption{Chế độ \textit{Chơi với Agent} trong Capture The Flag: người chơi phối hợp
...
theo kiểu hai người chơi chung bàn phím: người thứ nhất dùng cụm W A S D (Q/E đi
ngang), người thứ hai dùng cụm phím mũi tên (dấu ngoặc vuông để đi ngang). Chế độ
...
\caption{Chế độ \textit{Người chơi} trong Football Table: người chơi cầm đội Xanh,
...
% ───────────────────────────────────────────────
\subsection{Tích hợp mô hình ngôn ngữ lớn (LLM)}
\label{sub:app-llm}
...
\indent \textbf{Thể thức tương tác.} Tùy thuộc vào bản chất của từng môi trường, tương
...
\indent \textbf{Kiến trúc kết nối.} Giao tiếp giữa Unity và LLM được thực hiện qua
...
{\sloppy
\indent \textbf{Cấu hình khóa API.} Khóa API được đọc từ tệp
...
\caption{Chế độ \textit{Agent đấu LLM}: ô chọn mô hình RL và ô chọn LLM đối thủ; nút
bắt đầu bị khóa kèm cảnh báo khi chưa cấu hình khóa API}
\label{fig:app_menu_llm}
\end{figure}

\indent \textbf{Nhận xét về độ trễ và chất lượng hành vi.} Mạng nơ-ron ONNX chỉ mất
...
lên trong scene & Chưa dùng được \\
\hline
...
\indent Ứng dụng còn ba hạn chế đã biết. Thứ nhất, scene Cross The Road hiện chỉ giữ
lại một khu vực huấn luyện, trong khi hai chế độ đua song song cần ít nhất hai khu
vực; khắc phục chỉ đơn giản là nhân bản thêm khu vực trong scene, còn phần mã nguồn
chọn và đóng khung khu vực đã sẵn sàng cho số lượng khu vực bất kỳ. Thứ hai, chế độ
Agent đấu LLM phụ thuộc vào dịch vụ bên ngoài: cần khóa API hợp lệ, cần kết nối mạng,
và chất lượng chơi bị giới hạn bởi độ trễ một đến hai giây mỗi lượt. Thứ ba, hai tab
