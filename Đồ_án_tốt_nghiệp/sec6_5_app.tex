...
phải chọn được mô hình cụ thể (tương ứng PPO, SAC, MA-POCA, MAPPO, DQN, hoặc các phiên bản
huấn luyện khác nhau) cho tác nhân dùng. Yêu cầu này biến ứng dụng thành một công cụ
...
đúng phạm vi thực nghiệm của Chương~\ref{ch:results}: $15$ lần chạy, không thiếu thuật toán
nào. Người dùng nhờ vậy xem số liệu trước rồi chuyển sang tab chơi để đối chiếu
...
\texttt{Assets/GameHub}, tách hẳn khỏi mã nguồn ba môi trường; phần này thuộc một gói riêng
...
scene và cấu hình lại từng tác nhân theo chế độ đã chọn. Không, tức là ai đó mở scene trực
tiếp để huấn luyện, thì nó đứng ngoài, scene giữ nguyên cấu hình gốc. Chính cách
...
rồi chỉ sao chép các cặp lưới và vật liệu (\texttt{MeshFilter} và \texttt{MeshRenderer})
...
nhờ đó luôn đóng gói \textbf{toàn bộ $15$ mô hình} ứng với $15$ lần chạy của
...
từng môi trường chứ không cắt ở ba vị trí đầu, cho phép đọc ngay kết luận so sánh, kể cả thuật
toán xếp cuối, mà không cần nhìn biểu đồ.
...
như mọi bên khác. Còn người dùng gói, muốn thêm cách điều khiển mới (gamepad, kịch bản kiểm
thử tự động, hay chính một mô hình ngôn ngữ lớn) chỉ việc viết một
...
của ML-Agents có thể gọi \texttt{OnEpisodeBegin} (kéo theo việc đặt lại bàn và bóng) ngay
trong pha \texttt{OnEnable}, tức trước khi bộ khởi tạo của scene kịp xong. Kết quả là
