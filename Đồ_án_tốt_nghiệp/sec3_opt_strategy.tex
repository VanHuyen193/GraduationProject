...
Action Planning (GOAP) được giới thiệu và phổ biến qua tựa game \textit{F.E.A.R.}
...
duy trì đồng thời hai mạng nơ-ron: mạng \textit{actor} học policy $\pi_\theta(a \mid
s)$ và mạng \textit{critic} học hàm giá trị $V_\phi(s)$ hoặc $Q_\phi(s,a)$. Critic
...
\indent \textit{Ưu điểm:} PPO có tính ổn định huấn luyện cao nhờ cơ chế clipping,
...
\indent \textit{Nhược điểm:} Do là thuật toán on-policy, PPO không tái sử dụng được
...
\indent \textit{Ưu điểm:} SAC có hiệu quả mẫu vượt trội so với các thuật toán
...
\indent \textit{Nhược điểm:} SAC đòi hỏi bộ nhớ lớn hơn để lưu trữ replay buffer
...
PPO hay SAC nguyên bản khó xử lý tốt: bài toán \textit{gán công lao} (credit
assignment) --- xác định đóng góp của từng agent vào phần thưởng chung của cả nhóm ---
và bài toán \textit{tác nhân biến động} (posthumous credit) --- gán công lao đúng
...
\textit{huấn luyện tập trung, thực thi phân tán} (centralized training with
decentralized execution). Trong quá trình huấn luyện, một mạng critic tập trung
(centralized critic) được phép quan sát trạng thái và hành động của \textit{toàn bộ}
...
\indent \textit{Ưu điểm:} MA-POCA được thiết kế riêng cho bài toán hợp tác đa tác
...
\indent \textit{Nhược điểm:} Là thuật toán on-policy, MA-POCA cũng chịu hạn chế về
...
tín hiệu phần thưởng nội tại như \textit{curiosity} và \textit{RND} để hỗ trợ khám
